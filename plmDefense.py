...
def parse_args():
    ...
    parser.add_argument(
        "--config_path",
        type=str,
        default="./plmConfigs/Badnets4GA+LoRA+MSLRConfig.json",
    )
    ...
    parser.add_argument(
        ...
        choices=["local", "hf"],
        default=None,
        help="模型来源: local(本地) 或 hf(HuggingFace)",
    )
...
def main(config):
    ...
    logger.info(
        "Evaluate backdoored model on {}".format(config["target_dataset"]["name"])
    )
...
if __name__ == "__main__":
    ...
    if args.model_source == "local":
        config["victim"]["path"] = args.target_model
        config["victim"]["model"] = os.path.basename(args.target_model)
    elif args.model_source == "hf":
        config["victim"]["path"] = args.target_model
        config["victim"]["model"] = args.target_model
    else:
    ...
    if "muscleConfig" in config["victim"].keys():
        if config["victim"]["muscleConfig"].get("mslr"):
            ...
            if args.target_model.lower() in freqBands:
                config["victim"]["muscleConfig"]["mslrConfig"]["freqBand"] = freqBands[
                    args.target_model.lower()
                ]
    ...
    if args.dataset is not None:
        ...
        if args.dataset.lower() == "lingspam" and config["attacker"]["poisoner"][
            "name"
        ] in ["synbkd", "stylebkd"]:
